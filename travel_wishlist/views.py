...
def place_list(request):
    ...
    return render(request, 'travel_wishlist/wish_list.html', {'places':places, 'new_place_form': new_place_form})
...
def place_details(request, place_pk):
    ...
    #is this GET request (shoe data + form), or POST request(update Place object)
    ...
    if request.method =='POST':
        ...
        if form.is_valid():
            ...
            messages.error(request, form.errors) #temporaary refine later

        return redirect('place_details', place_pk=place_pk)


    else: # if GEt request, show Place info and form and optional form.
